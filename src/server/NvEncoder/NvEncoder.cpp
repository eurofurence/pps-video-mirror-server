...
void NvEncoder::EncodeFrame(std::vector<std::vector<std::byte>> &vPacket, NV_ENC_PIC_PARAMS *pPicParams)
{
    vPacket.clear();
...
void NvEncoder::RunMotionEstimation(std::vector<std::byte> &mvData)
{
    if (!m_hEncoder)
    {
        NVENC_THROW_ERROR("Encoder Initialization failed", NV_ENC_ERR_NO_ENCODE_DEVICE);
    ...
    {
        m_iToSend++;
        std::vector<std::vector<std::byte>> vPacket;
...
void NvEncoder::GetSequenceParams(std::vector<std::byte> &seqParams)
{
    std::byte spsppsData[1024]; // Assume maximum spspps data is 1KB or less
...
void NvEncoder::EndEncode(std::vector<std::vector<std::byte>> &vPacket)
{
    vPacket.clear();
    if (!IsHWEncoderInitialized())
    {
...
void NvEncoder::GetEncodedPacket(std::vector<NV_ENC_OUTPUT_PTR> &vOutputBuffer, std::vector<std::vector<std::byte>> &vPacket, bool bOutputDelay)
{
    unsigned i = 0;
    ...
    for (; m_iGot < iEnd; m_iGot++)
    {
        ...
        std::byte *pData = (std::byte *)lockBitstreamData.bitstreamBufferPtr;
        if (vPacket.size() < i + 1)
        {
            vPacket.push_back(std::vector<std::byte>());
        }
        vPacket[i].clear();
